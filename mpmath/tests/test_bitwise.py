...
def test_round_half_even():
    assert from_man_exp(0, -4, 4, round_half_even)[:3] == (0, 0, 0)
    assert from_man_exp(0xf0, -4, 4, round_half_even)[:3] == (0, 15, 0)
    assert from_man_exp(0xf7, -4, 4, round_half_even)[:3] == (0, 15, 0)
    assert from_man_exp(0xf8, -4, 4, round_half_even)[:3] == (0, 1, 4)    # 1111.1000 -> 10000.0
    assert from_man_exp(0xf9, -4, 4, round_half_even)[:3] == (0, 1, 4)    # 1111.1001 -> 10000.0
    assert from_man_exp(0xe8, -4, 4, round_half_even)[:3] == (0, 7, 1)    # 1110.1000 -> 1110.0
    assert from_man_exp(0xe9, -4, 4, round_half_even)[:3] == (0, 15, 0)     # 1110.1001 -> 1111.0
    assert from_man_exp(-0xf0, -4, 4, round_half_even)[:3] == (1, 15, 0)
    assert from_man_exp(-0xf7, -4, 4, round_half_even)[:3] == (1, 15, 0)
    assert from_man_exp(-0xf8, -4, 4, round_half_even)[:3] == (1, 1, 4)
    assert from_man_exp(-0xf9, -4, 4, round_half_even)[:3] == (1, 1, 4)
    assert from_man_exp(-0xe8, -4, 4, round_half_even)[:3] == (1, 7, 1)
    assert from_man_exp(-0xe9, -4, 4, round_half_even)[:3] == (1, 15, 0)

def test_rounding_bugs():
    # 1 less than power-of-two cases
    assert from_man_exp(72057594037927935, -56, 53, round_up) == (0, 1, 0, 1)
    assert from_man_exp(73786976294838205979l, -65, 53, round_half_even) == (0, 1, 1, 1)

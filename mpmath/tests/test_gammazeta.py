...
def test_bernoulli(plus):
    ...
    assert str(bernoulli(10, plus)) == '0.07575757575757576'
    assert repr(bernoulli(10, plus)) == "mpf('0.07575757575757576')"
    assert str(bernoulli(234, plus)) == '7.6277279396434395e+267'
    assert str(bernoulli(10**5, plus)) == '-5.822294314613351e+376755'
    assert str(bernoulli(10**8+2, plus)) == '1.1957035503995297e+676752584'

    mp.dps = 50
    assert str(bernoulli(10, plus)) == '0.0757575757575757575757575757575757575757575757575757'
    assert str(bernoulli(234, plus)) == '7.62772793964343924869949690204961215533858633733309e+267'
    assert str(bernoulli(10**5, plus)) == '-5.8222943146133508236497045360612887555320691004308e+376755'
    assert str(bernoulli(10**8+2, plus)) == '1.195703550399529727226304788460434691460208831778238e+676752584'
...
def test_zeta_huge():
    ...
    v = zeta(33000)
    mp.dps = 15
    assert str(v-1) == '1.0236301959811827e-9934'
...
def test_zeta_negative():
    ...
    a = -pi*10**40
    mp.dps = 15
    assert str(zeta(a)) == '2.5588049270871156e+1233536161668617575553892558646631323374078'
    mp.dps = 50
    assert str(zeta(a)) == '2.55880492708711549608750333373844320384363308473331e+1233536161668617575553892558646631323374078'
...
def test_polygamma_high_prec():
    mp.dps = 100
    assert str(psi(0,pi)) == "0.977213307942006733292069486406182343640834609994325638009523286531810592477714131730207565436292873435"
    assert str(psi(10,pi)) == "-12.9887618143488952931028376941422258830717596221370717077380355051830761776965756274717410190065923765"
...
def test_polygamma_high_order():
    mp.dps = 100
    assert str(psi(50, pi)) == "-1344100348958402765749252447726432491812.64198527316053105570709598922789775303582315239767962613648304"
    assert str(psi(50, pi + 14*e)) == "-1.89793739550804321623512073101895801993019919886375952881053090844591920308111549337295143780341395978e-18"
    assert str(psi(50, pi + 14*e*j)) == \
    ("(-5.22516941152169248975225472155683565752375889510631513244785937738523370009487"
     "125650781415195662443306e-17-1.81315704140701018470241411021820534852786219632798"
     "041775766528224472896389129808019934148088181161295e-17j)")
    mp.dps = 15
    assert str(psi(50, pi)) == "-1.3441003489584055e+39"
    assert str(psi(50, pi + 14*e)) == "-1.897937395508038e-18"
    assert str(psi(50, pi + 14*e*j)) == "(-5.2251694115217006e-17-1.8131570414070127e-17j)"
...
def test_gamma_huge_1():
    ...
    x = mpf(10**10) / 7
    mp.dps = 15
    assert str(gamma(x)) == "6.260753213895193e+12458010678"
    mp.dps = 50
    assert str(gamma(x)) == "6.26075321389519292013037792917074558740104207839331e+12458010678"
...
def test_gamma_huge_2():
    ...
    mp.dps = 15
    assert str(gamma(x)) == (\
        "1.8234113477667888e+5172997469323364168990133558175077136829182824042201886051511"
        "9656908623426021308685461258226190190661")
    mp.dps = 50
    assert str(gamma(x)) == (\
        "1.823411347766788753744149103500275969399804129839975e+5172997469323364168990133558"
...
def test_gamma_huge_3():
    ...
    y = gamma(x)
    assert str(y.real) == (\
        "-6.829252039181055e+2636286142112569524501781477865238132302397236429627932441916"
        "056964386399485392600")
    assert str(y.imag) == (\
        "8.546471436784175e+26362861421125695245017814778652381323023972364296279324419160"
        "56964386399485392600")
    mp.dps = 50
    y = gamma(x)
    assert str(y.real) == (\
        "-6.82925203918105484606827362267996373560165384218172e+26362861421125695245017814"
        "77865238132302397236429627932441916056964386399485392600")
    assert str(y.imag) == (\
        "8.54647143678417485074793069481306875117114202340147e+263628614211256952450178147"
        "7865238132302397236429627932441916056964386399485392600")

def test_gamma_huge_4():
    x = 3200+11500j
    assert str(gamma(x)) == \
        "(8.957832685397133e+5164-1.946787983297351e+5164j)"
    mp.dps = 50
    assert str(gamma(x)) == (\
        "(8.95783268539713395702929526976755708222065673270922e+5164"
        "-1.946787983297350956889540213942964365032952414479405e+51"
        "64j)")
...
def test_gamma_huge_5():
    ...
    mp.dps = 15
    y = gamma(x)
    assert str(y.real) == "-3.277538996349413e-227396058973640224580963937571892628368354580620654233316839"
    assert str(y.imag) == "-7.151988895041598e-227396058973640224580963937571892628368354580620654233316841"
    mp.dps = 50
    y = gamma(x)
    assert str(y.real) == (\
        "-3.277538996349413216895005699597469094698321912393495e-22739605897364022458096393"
        "7571892628368354580620654233316839")
    assert str(y.imag) == (\
        "-7.15198889504159797497367492225302097131365888858973e-22739605897364022458096393"
...
def test_gamma_huge_7():
    ...
    assert str(y.imag) == "1.8455686701969344e-1000"
    mp.dps = 50
    y = gamma(a)
    assert str(y.real) == "2.0"
    assert str(y.imag) == "1.845568670196934278786975819835195137915681328120152e-1000"
...
def test_barnesg():
    ...
    assert str(superfac(1000)) == '3.2457081842236783e+1177245'

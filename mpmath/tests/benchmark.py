...
def getrandom(type):
    ...
    if type == 'mpfval':
        return (mpf(random() * 2.0**randint(-10, 10)) ** 0.5)._mpf_
    if type == 'Decimal':
...
atests = [
  ...
  ('Integer power (x^5)', 'x**5', 'x**5', 'fpowi(x, 5, prec, RF)'),
  ('Exponential function (exp(x))', 'x.exp()', 'exp(x)', 'fexp(x, prec, RF)'),
  ('Natural logarithm (log(x))', 'x.ln()', 'log(x)', 'flog(x, prec, RF)'),

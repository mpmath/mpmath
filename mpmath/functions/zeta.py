...
def secondzeta(ctx, s, a = 0.015, *, verbose=False, error=False):
    ...
    **Examples**
        ...
        >>> mp.pretty = True
        >>> secondzeta(2)
        0.023104993115418977
        >>> xi = lambda s: 0.5*s*(s-1)*pi**(-0.5*s)*gamma(0.5*s)*zeta(s)
        >>> Xi = lambda t: xi(0.5+t*j)
        >>> chop(-0.5*diff(Xi,0,n=2)/Xi(0))
        0.023104993115418974

    We may ask for an approximate error value::

        >>> secondzeta(0.5+100j, error=True)
        ((-0.21627201127671758-0.8449527089372278j), 2.220446049250313e-16)
    ...
    **Implementation notes**
    ...
    values of the four terms.
        ...
        main term = (-30190318549.1386563125557-13964804384.62462287652292j)
            computed using 19 zeros of zeta
        prime term = (132717176.892127546250448+188980555.1756397829060072j)
            computed using 9 values of the von Mangoldt function
        exponential term = (542447428666.0717981253574+362434922978.8019243520334j)
        singular term = (512124392939.9815432235546+348281138038.6553102392095j)
        ((0.059471042997+0.346351453394j), 1.45519152284e-11)

        >>> secondzeta(0.5 + 40j, a=0.04, error=True, verbose=True)
        main term = (-151962888.1960624390772457-217930683.9021029405198243j)
            computed using 9 zeros of zeta
        prime term = (2476659342.303872237246097+28711581821.92162716313578j)
            computed using 37 values of the von Mangoldt function
        exponential term = (178506047114.7838188263977+819674143244.456773305757j)
        singular term = (175877424884.2244131070772+790744630738.286691748707j)
        ((0.059471042997+0.346351453394j), 1.45519152284e-11)
...
def lerchphi(ctx, z, s, a):
    ...
    **Examples**
    ...
    Several evaluations in terms of simpler functions::
        ...
        >>> mp.pretty = True
        >>> lerchphi(-1,2,0.5)
        3.66386237670887606021841407
        >>> 4*catalan
        3.66386237670887606021841407
        >>> diff(lerchphi, (-1,-2,1), (0,1,0))
        0.213139199408752895461760712
        >>> 7*zeta(3)/(4*pi**2)
        0.21313919940875289546176071
        >>> lerchphi(-4,1,1)
        0.402359478108525093650189836
        >>> log(5)/4
        0.402359478108525093650189836
        >>> lerchphi(-3+2j,1,0.5)
        (1.14242344712025713777400178+0.211823238098020135049579546j)
        >>> 2*atanh(sqrt(-3+2j))/sqrt(-3+2j)
        (1.14242344712025713777400178+0.21182323809802013504957955j)

    Evaluation works for complex arguments and `|z| \ge 1`::

        >>> lerchphi(1+2j, 3-j, 4+2j)
        (0.0020250099570099086005394687+0.00332789753681355880743808906j)
        >>> lerchphi(-2,2,-2.5)
        -12.286762723530942752659443
        >>> lerchphi(10,10,10)
        (-4.4621307271021857018173488e-11-1.57517219898109621882348116e-12j)
        >>> lerchphi(10,10,-10.5)
        (112658784011940.56057890025-498113185.575622177774363114j)
    ...
    Reduction to simpler functions::
        ...
        (1.04467445755674666803397476-0.0467450865401265893227122645j)
        >>> zeta(4.25+1j)
        (1.04467445755674666803397476-0.0467450865401265893227122645j)
        >>> lerchphi(1 - 0.5**10, 4.25+1j, 1)
        (1.04462933802150754673719651-0.046677688139633881817081005j)
        >>> lerchphi(3, 4, 1)
        (1.24950329702336654519259243-0.231425241337566477647446238j)
        >>> polylog(4, 3) / 3
        (1.2495032970233665451925924-0.23142524133756647764744624j)
        >>> lerchphi(3, 4, 1 - 0.5**10)
        (1.25397806394666394567267397-0.231673662283653546876537602j)

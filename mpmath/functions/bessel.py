...
def spherical_jn(ctx, n, z):
    ...
    **Examples**
    ...
    """
    return ctx.besselj(n + ctx.one/2, z) * ctx.sqrt(ctx.pi/(2*z)) if z != ctx.zero else ctx.one
...
def spherical_yn(ctx, n, z):
    ...
    **Examples**
    ...
    """
    return ctx.bessely(n + ctx.one/2, z) * ctx.sqrt(ctx.pi/(2*z)) if z != ctx.zero else ctx.ninf

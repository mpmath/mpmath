...
def pcfd(ctx, n, z, *, eliminate=True, eliminate_all=False,
    ...
    **Examples**
        ...
        3.0
        >>> pcfd(-3,0)
        0.62665706865775012560394132
        >>> pcfd('1/2', 2+3j)
        (-5.3633311612329207348490563-3.8588778217900107141634873j)
        >>> pcfd(2, -10)
        1.37490644263143803887151462e-09
...
def pcfu(ctx, a, z, *, eliminate=True, eliminate_all=False,
    ...
    **Examples**
    ...
    Connection to other functions::
        ...
        >>> z = mpf(3)
        >>> pcfu(0.5,z)
        0.032103581293111514505519628
        >>> sqrt(pi/2)*exp(z**2/4)*erfc(z/sqrt(2))
        0.0321035812931115145055196264
        >>> pcfu(0.5,-z)
        23.750123328352972337112549
        >>> sqrt(pi/2)*exp(z**2/4)*erfc(-z/sqrt(2))
        23.7501233283529723371125493
        >>> pcfu(0.5,-z)
        23.750123328352972337112549
        >>> sqrt(pi/2)*exp(z**2/4)*erfc(-z/sqrt(2))
        23.7501233283529723371125493
...
def pcfv(ctx, a, z, *, eliminate=True, eliminate_all=False,
    ...
    **Examples**
    ...
    Wronskian relation between `U` and `V`::
        ...
        0.79788456080286535587989212
        >>> sqrt(2/pi)
        0.79788456080286535587989212
        >>> a, z = 2.5, 3
        >>> pcfu(a,z)*diff(pcfv,(a,z),(0,1))-diff(pcfu,(a,z),(0,1))*pcfv(a,z)
        0.79788456080286535587989211
        >>> a, z = 0.25, -1
        >>> pcfu(a,z)*diff(pcfv,(a,z),(0,1))-diff(pcfu,(a,z),(0,1))*pcfv(a,z)
        0.79788456080286535587989211
        >>> a, z = 2+1j, 2+3j
...
def pcfw(ctx, a, z, *, eliminate=True, eliminate_all=False,
    ...
    **Examples**
    ...
    Value at the origin::
        ...
        >>> a = mpf(0.25)
        >>> pcfw(a,0)
        0.97228332457181807656171037
        >>> power(2,-0.75)*sqrt(abs(gamma(0.25+0.5j*a)/gamma(0.75+0.5j*a)))
        0.97228332457181807656171039
        >>> diff(pcfw,(a,0),(0,1))
        -0.51425339442100789660036241

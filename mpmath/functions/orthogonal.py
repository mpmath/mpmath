...
def pcfd(ctx, n, z, **kwargs):
    ...
    **Examples**
        ...
        >>> pcfd(4,0); pcfd(-3,0)
        3.0
        0.6266570686577501256039413217
        >>> pcfd('1/2', 2+3j)
        (-5.363331161232920734849056306 - 3.85887782179001071416348731j)
        >>> pcfd(2, -10)
        1.374906442631438038871514619e-9
...
def pcfu(ctx, a, z, **kwargs):
    ...
    **Examples**
    ...
    Connection to other functions::
        ...
        >>> z = mpf(3)
        >>> pcfu(0.5,z)
        0.03210358129311151450551962805
        >>> sqrt(pi/2)*exp(z**2/4)*erfc(z/sqrt(2))
        0.03210358129311151450551962643
        >>> pcfu(0.5,-z)
        23.75012332835297233711254887
        >>> sqrt(pi/2)*exp(z**2/4)*erfc(-z/sqrt(2))
        23.75012332835297233711254929
        >>> pcfu(0.5,-z)
        23.75012332835297233711254887
        >>> sqrt(pi/2)*exp(z**2/4)*erfc(-z/sqrt(2))
        23.75012332835297233711254929
...
def pcfv(ctx, a, z, **kwargs):
    ...
    **Examples**
    ...
    Wronskian relation between `U` and `V`::
        ...
        0.7978845608028653558798921247
        >>> sqrt(2/pi)
        0.7978845608028653558798921247
        >>> a, z = 2.5, 3
        >>> pcfu(a,z)*diff(pcfv,(a,z),(0,1))-diff(pcfu,(a,z),(0,1))*pcfv(a,z)
        0.7978845608028653558798921118
        >>> a, z = 0.25, -1
        >>> pcfu(a,z)*diff(pcfv,(a,z),(0,1))-diff(pcfu,(a,z),(0,1))*pcfv(a,z)
        0.7978845608028653558798921118
        >>> a, z = 2+1j, 2+3j
...
def pcfw(ctx, a, z, **kwargs):
    ...
    **Examples**
    ...
    Value at the origin::
        ...
        >>> a = mpf(0.25)
        >>> pcfw(a,0)
        0.9722833245718180765617103756
        >>> power(2,-0.75)*sqrt(abs(gamma(0.25+0.5j*a)/gamma(0.75+0.5j*a)))
        0.9722833245718180765617103874
        >>> diff(pcfw,(a,0),(0,1))
        -0.5142533944210078966003624113

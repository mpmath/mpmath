...
def eta(ctx, tau):
    ...
    Returns the Dedekind eta function of tau in the upper half-plane.
        ...
        >>> mp.pretty = True
        >>> eta(1j)
        (0.76822542232605665900259418+0j)
        >>> gamma(0.25) / (2*pi**0.75)
        0.76822542232605665900259418
        >>> tau = sqrt(2) + sqrt(5)*1j
        >>> eta(-1/tau)
        (0.90228599084393764635732944+0.079850936739480984080485747j)
        >>> sqrt(-1j*tau) * eta(tau)
        (0.90228599084393764635732945+0.079850936739480984080485745j)
        >>> eta(tau+1)
        (0.449306613971755378622311384+0.32900147938779866639159393j)
        >>> exp(pi*1j/12) * eta(tau)
        (0.44930661397175537862231139+0.329001479387798666391593934j)
...
def qfrom(ctx, q=None, m=None, k=None, tau=None, qbar=None,
    ...
    or both half-periods `\omega_1, \omega_2`::
        ...
        (0.20787957635076190854695562+0j)
...
def taufrom(ctx, q=None, m=None, k=None, tau=None, qbar=None,
    ...
    **Examples**
        ...
        0.50000000000000000000000001j
        >>> taufrom(k=kfrom(tau=0.5j))
        0.499999999999999999999999974j
...
def kfrom(ctx, q=None, m=None, k=None, tau=None, qbar=None,
    ...
    or both half-periods `\omega_1, \omega_2`::
        ...
        (0.98517143100941603868950197+0j)

    As `q \to 1` and `q \to -1`, `k` rapidly approaches
    `1` and `i \infty` respectively::

        >>> kfrom(q=0.75)
        0.99999999999998991664717666
        >>> kfrom(q=-0.75)
        7041781.0966920383327906146j
        >>> kfrom(q=1)
        1
        >>> kfrom(q=-1)
        infj
...
def mfrom(ctx, q=None, m=None, k=None, tau=None, qbar=None,
    ...
    or both half-periods `\omega_1, \omega_2`::
        ...
        (0.249999999999999999999999997+0j)
        >>> mfrom(qbar=qbarfrom(m=0.25))
        0.25
        >>> mfrom(omega1=1, omega2=0.5j)
        (0.97056274847714058562026469+0j)

    As `q \to 1` and `q \to -1`, `m` rapidly approaches
    `1` and `-\infty` respectively::

        >>> mfrom(q=0.75)
        0.99999999999997983329435332
        >>> mfrom(q=-0.75)
        -49586681013729.326115583532
        >>> mfrom(q=1)
        1.0
        >>> mfrom(q=-1)
        -inf
...
def kleinj(ctx, tau=None, q=None, m=None, k=None, qbar=None,
    ...
    **Examples**
    ...
    Verifying the functional equation `J(\tau) = J(\tau+1) = J(-\tau^{-1})`::
        ...
        >>> tau = 0.625+0.75*j
        >>> kleinj(tau)
        (-0.150749216651118226712524197+0.075959483790845719272289476j)
        >>> kleinj(tau+1)
        (-0.150749216651118226712524197+0.075959483790845719272289476j)
        >>> kleinj(-1/tau)
        (-0.150749216651118226712524187+0.075959483790845719272289462j)
    ...
    `\bar{q}`, `j(\tau) = \bar{q}^{-1} + 744 + 196884\bar{q} + \ldots`::
        ...
        [1.0, 743.99999999999999999999997, 196883.999999999999999999997, 21493760.0, 864299970.0, 20245856256.0]
    ...
    algebraic values, e.g.::
        ...
        1264538.90947514050932022704
        >>> identify(cbrt(_))      # note: not simplified
        '((100+sqrt(13520))/2)'
        >>> (50+26*sqrt(5))**3
        1264538.90947514050932022706
...
def kleinjinv(ctx, J):
    ...
    **Examples**
        ...
        (-0.200000000000000000000000016+1.59999999999999999999999994j)
...
def elliprf(ctx, x, y, z):
    ...
    **Examples**
    ...
    Some symmetries and argument transformations::
        ...
        >>> k = mpf(100000)
        >>> elliprf(k*x,k*y,k*z)
        0.0018470321219233212532192837
        >>> k**(-0.5) * elliprf(x,y,z)
        0.0018470321219233212532192837
    ...
    on `t \in (0, 1/2)`::
        ...
        >>> x,y,z = j-1,j,0
        >>> elliprf(x,y,z)
        (0.79612586584233913293056938-1.21385666983649598643009426j)
...
def elliprc(ctx, x, y, pv=True):
    ...
    **Examples**
    ...
    Comparing with numerical integration::
        ...
        (0.3333969101113672670749334+0.70248147310407263931563747j)
...
def elliprj(ctx, x, y, z, p, integration=1):
    ...
    **Examples**
    ...
    Some values and limits::
        ...
        1.0
        >>> elliprj(2,2,2,2)
        0.35355339059327376220042218
        >>> 1/(2*sqrt(2))
        0.353553390593273762200422185
        >>> elliprj(0,1,2,2)
        1.06793798966739570226868782
        >>> 3*(2*gamma('5/4')**2-pi**2/gamma('1/4')**2)/(sqrt(2*pi))
        1.06793798966739570226868782
        >>> elliprj(0,1,1,2)
        1.38022677676591517243205434
        >>> 3*pi*(2-sqrt(2))/4
        1.38022677676591517243205437
        >>> elliprj(1,3,2,0)
        inf
        >>> elliprj(0,1,1,0)
        inf
    ...
    Scale transformation::
        ...
        4.5212916775927455278511679e-09
    ...
    Comparing with numerical integration::
        ...
        >>> elliprj(1,2,3,4)
        0.23984809974956776217586167
        >>> f = lambda t: 1/((t+4)*sqrt((t+1)*(t+2)*(t+3)))
        >>> 1.5*quad(f, [0,inf])
        0.23984809974956776217586167
        >>> elliprj(1,2+1j,3,4-2j)
        (0.216888906014633498739951965+0.040819126273666733323695115j)
        >>> f = lambda t: 1/((t+4-2j)*sqrt((t+1)*(t+2+1j)*(t+3)))
        >>> 1.5*quad(f, [0,inf])
        (0.216888906014633498739951962+0.040819126273666733323695114j)
...
def elliprg(ctx, x, y, z):
    ...
    **Examples**
    ...
    Evaluation for real and complex arguments::
        ...
        3.1415926535897932384626434
        >>> elliprg(0,0.5,1)
        0.67532194052383775126008737
        >>> chop(elliprg(1+j, 1-j, 2))
        1.17243132767641660453282156
...
def ellipf(ctx, phi, m):
    ...
    **Examples**
    ...
    Basic values and limits::
        ...
        1.22619117088351707081306096
        >>> ellipf(pi/2, -0.5)
        1.41573720842595619889216599
        >>> ellipk(-0.5)
        1.41573720842595619889216596
        >>> ellipf(pi/2+eps, 1)
        inf
        >>> ellipf(-pi/2-eps, 1)
        inf
        >>> ellipf(1.5, 1)
        3.34067754279831100332081265
    ...
    The arguments may be complex numbers::
        ...
        >>> ellipf(3j, 0.5)
        1.71360240784159023480414326j
        >>> ellipf(3+4j, 5-6j)
        (1.2691312419503513233057407-0.356105281501455833541253775j)
        >>> z,m = 2+3j, 1.25
        >>> k = 1011
        >>> ellipf(z+pi*k,m)
        (4086.18438362217976408282144-3003.00353892374939654687064j)
        >>> ellipf(z,m) + 2*k*ellipk(m)
        (4086.1843836221797640828214-3003.00353892374939654687064j)
...
def ellipe(ctx, *args):
    ...
    **Examples for the complete integral**
    ...
    Basic values and limits::
        ...
        1.0
        >>> ellipe(-1)
        1.91009889451385600895238104
        >>> ellipe(2)
        (0.59907011736779610371996125+0.59907011736779610371996125j)
        >>> ellipe(inf)
        infj
        >>> ellipe(-inf)
        inf
    ...
    Evaluation is supported for arbitrary complex `m`::
        ...
        >>> ellipe(0.5+0.25j)
        (1.36086868216312968271668675-0.123873344256178684355731509j)
        >>> ellipe(3+4j)
        (1.49955352093334695433361223-1.57787900791275827453330894j)

    A definite integral::

        >>> quad(ellipe, [0,1])
        1.33333333333333333333333334
    ...
    **Examples for the incomplete integral**
    ...
    Basic values and limits::
        ...
        -1.0
        >>> ellipe(1.5, 1)
        0.99749498660405443094172337

    Comparing with numerical integration::

        >>> z,m = 0.5, 1.25
        >>> ellipe(z,m)
        0.47401521826526283942644492
    ...
    The arguments may be complex numbers::
        ...
        >>> ellipe(3j, 0.5)
        7.5519912348903718735021048j
        >>> ellipe(3+4j, 5-6j)
        (24.1529902257422050242446633+75.250367048032599741815599j)
        >>> k = 35
        >>> z,m = 2+3j, 1.25
        >>> ellipe(z+pi*k,m)
        (48.301387994120052350907658+17.4725521672198768822435686j)
        >>> ellipe(z,m) + 2*k*ellipe(m)
        (48.301387994120052350907657+17.4725521672198768822435677j)
    ...
    (see :func:`~mpmath.appellf1`)::
        ...
        >>> z,m = 0.5, 0.25
        >>> ellipe(z,m)
        0.49500170301641519288703755
        >>> sin(z)*appellf1(0.5,0.5,-0.5,1.5,sin(z)**2,m*sin(z)**2)
        0.495001703016415192887037554
...
def ellippi(ctx, *args):
    ...
    **Examples for the complete integral**
    ...
    Evaluation in terms of simpler functions::
        ...
        >>> ellippi(0.25,0.25)
        1.95661627911923620727972703
        >>> ellipe(0.25)/(1-0.25)
        1.956616279119236207279727
        >>> ellippi(3,0)
        -1.11072073453959156175397025j
        >>> pi/(2*sqrt(-2))
        -1.11072073453959156175397027j
        >>> ellippi(-3,0)
    ...
    **Examples for the incomplete integral**
    ...
    Basic values and limits::
        ...
        0.0
        >>> ellippi(0,0.25,0.5)
        0.25130400865449257941345913
        >>> ellipf(0.25,0.5)
        0.25130400865449257941345913
        >>> ellippi(1,1,1)
        2.0543329332562486686924519
        >>> (log(sec(1)+tan(1))+sec(1)*tan(1))/2
        2.05433293325624866869245185
        >>> ellippi(0.25, 53*pi/2, 0.75)
        135.240868757890840755057957
        >>> 53*ellippi(0.25,0.75)
        135.240868757890840755057954
        >>> ellippi(0.5,pi/4,0.5)
        0.91902273916569699039872692
        >>> 2*ellipe(pi/4,0.5)-1/sqrt(3)
        0.91902273916569699039872693

    Complex arguments are supported::

        >>> ellippi(0.5, 5+6j-2*pi, -7-8j)
        (-0.36128566200767476604101671+0.5217735339984807829755815j)
...
def g2g3from(ctx, q=None, m=None, k=None, tau=None, qbar=None,
    ...
    `\omega_2 = \tau/2`::
        ...
        (2079.7999214215724, -18198.74117609861)
...
def omega1omega2from(ctx, q=None, m=None, k=None, tau=None, qbar=None,
    ...
    the negative imaginary axis included as its boundary::
        ...
        >>> chop(g2), chop(g3)
        (59.99999999999998, 139.99999999999994)
        >>> chop(omega2/omega1)
        (0.5+1.1959878466430198j)

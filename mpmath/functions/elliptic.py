...
def eta(ctx, tau):
    ...
    Returns the Dedekind eta function of tau in the upper half-plane.
        ...
        (0.7682254223260566590025941786 + 0.0j)
        0.7682254223260566590025941786
        >>> tau = sqrt(2) + sqrt(5)*1j
        >>> eta(-1/tau); sqrt(-1j*tau) * eta(tau)
        (0.9022859908439376463573294408 + 0.07985093673948098408048574687j)
        (0.9022859908439376463573294537 + 0.07985093673948098408048574525j)
        >>> eta(tau+1); exp(pi*1j/12) * eta(tau)
        (0.4493066139717553786223113841 + 0.329001479387798666391593928j)
        (0.4493066139717553786223113906 + 0.3290014793877986663915939344j)
...
def qfrom(ctx, q=None, m=None, k=None, tau=None, qbar=None):
    ...
    Returns the elliptic nome `q`, given any of `q, m, k, \tau, \bar{q}`::
        ...
        0.2500000000000000000000000194
...
def qbarfrom(ctx, q=None, m=None, k=None, tau=None, qbar=None):
    ...
    `q, m, k, \tau, \bar{q}`::
        ...
        0.2500000000000000000000000065
...
def taufrom(ctx, q=None, m=None, k=None, tau=None, qbar=None):
    ...
    `q, m, k, \tau, \bar{q}`::
        ...
        (0.0 + 0.5000000000000000000000000129j)
        >>> taufrom(k=kfrom(tau=0.5j))
        (0.0 + 0.4999999999999999999999999742j)
...
def kfrom(ctx, q=None, m=None, k=None, tau=None, qbar=None):
    ...
    `1` and `i \infty` respectively::
        ...
        >>> kfrom(q=0.75)
        0.9999999999999899166471766583
        >>> kfrom(q=-0.75)
        (0.0 + 7041781.096692038332790614628j)
        >>> kfrom(q=1)
        1
        >>> kfrom(q=-1)
        (0.0 + infj)
...
def mfrom(ctx, q=None, m=None, k=None, tau=None, qbar=None):
    ...
    `q, m, k, \tau, \bar{q}`::
        ...
        (0.2499999999999999999999999968 + 0.0j)
    ...
    `1` and `-\infty` respectively::
        ...
        >>> mfrom(q=0.75)
        0.9999999999999798332943533166
        >>> mfrom(q=-0.75)
        -49586681013729.32611558353165
        >>> mfrom(q=1)
        1.0
        >>> mfrom(q=-1)
        -inf
...
def kleinj(ctx, tau=None, **kwargs):
    ...
    **Examples**
    ...
    Verifying the functional equation `J(\tau) = J(\tau+1) = J(-\tau^{-1})`::
        ...
        >>> tau = 0.625+0.75*j
        >>> kleinj(tau)
        (-0.1507492166511182267125241965 + 0.07595948379084571927228947639j)
        >>> kleinj(tau+1)
        (-0.1507492166511182267125241965 + 0.07595948379084571927228947639j)
        >>> kleinj(-1/tau)
        (-0.1507492166511182267125241868 + 0.07595948379084571927228946185j)
    ...
    `\bar{q}`, `j(\tau) = \bar{q}^{-1} + 744 + 196884\bar{q} + \ldots`::
        ...
        [1.0, 743.99999999999977, 196883.99999999997, 21493760.0, 864299970.0, 20245856256.0]
    ...
    related to the Heegner numbers 1, 2, 3, 7, 11, 19, 43, 67, 163::
        ...
        ...
        (1, 1728.0)
        (2, 7999.999999999999999999999894)
        (3, 0.0)
        (7, -3375.0)
        (11, -32768.0)
        (19, -884736.0)
    ...
    algebraic values, e.g.::
        ...
        1264538.909475140509320227036
        >>> identify(cbrt(_))      # note: not simplified
        '((100+sqrt(13520))/2)'
        >>> (50+26*sqrt(5))**3
        1264538.909475140509320227063
...
def elliprf(ctx, x, y, z):
    ...
    **Examples**
    ...
    Some basic values and limits::
        ...
        1.0
        >>> elliprf(2,2,2)**2
        0.4999999999999999999999999935
    ...
    Representing complete elliptic integrals in terms of `R_F`::
        ...
        2.156515647499643235438675017
        2.156515647499643235438675017
        >>> ellipe(m); elliprf(0,1-m,1)-m*elliprd(0,1-m,1)/3
        1.21105602756845952480356291
        1.21105602756845952480356291
    ...
    Some symmetries and argument transformations::
        ...
        0.5840828416771517066928491733
        0.5840828416771517066928491733
        0.5840828416771517066928491733
        >>> k = mpf(100000)
        >>> elliprf(k*x,k*y,k*z); k**(-0.5) * elliprf(x,y,z)
        0.001847032121923321253219283704
        0.001847032121923321253219283704
    ...
    Comparing with numerical integration::
        ...
        >>> q = extradps(25)(quad)
        >>> q(f, [0,inf])
        0.5840828416771517066928491682
    ...
    on `t \in (0, 1/2)`::
        ...
        >>> x,y,z = j-1,j,0
        >>> elliprf(x,y,z)
        (0.7961258658423391329305693829 - 1.213856669836495986430094261j)
    ...
    The so-called *first lemniscate constant*, a transcendental number::
        ...
        >>> elliprf(0,1,2)
        1.311028777146059905232419783
        >>> extradps(25)(quad)(lambda t: 1/sqrt(1-t**4), [0,1])
        1.311028777146059905232419794
        >>> gamma('1/4')**2/(4*sqrt(2*pi))
        1.311028777146059905232419809
...
def elliprc(ctx, x, y, pv=True):
    ...
    **Examples**
    ...
    Some special values and limits::
        ...
        inf
        >>> elliprc(5,5)**2
        0.1999999999999999999999999994
    ...
    Comparing with the elementary closed-form solution::
        ...
        2.041630778983498390751237994
        2.041630778983498390751237994
        >>> elliprc('1/5', '1/3'); sqrt(7.5)*acos(sqrt('3/5'))
        1.875180765206547065111085311
        1.875180765206547065111085311
    ...
    Comparing with numerical integration::
        ...
        >>> q = extradps(25)(quad)
        >>> elliprc(2, -3, pv=True)
        0.3333969101113672670749334029
        >>> elliprc(2, -3, pv=False)
        (0.3333969101113672670749334029 + 0.7024814731040726393156374663j)
...
def elliprj(ctx, x, y, z, p, integration=1):
    ...
    **Examples**
    ...
    Some values and limits::
        ...
        0.3535533905932737622004221783
        0.3535533905932737622004221848
        >>> elliprj(0,1,2,2)
        1.067937989667395702268687824
        >>> 3*(2*gamma('5/4')**2-pi**2/gamma('1/4')**2)/(sqrt(2*pi))
        1.067937989667395702268687824
        >>> elliprj(0,1,1,2); 3*pi*(2-sqrt(2))/4
        1.380226776765915172432054342
        1.380226776765915172432054368
        ...
        0.8505007163686739432927844064
    ...
    Scale transformation::
        ...
        4.521291677592745527851167889e-9
        4.521291677592745527851167889e-9

    Comparing with numerical integration::

        >>> elliprj(1,2,3,4)
        0.2398480997495677621758616696
        >>> f = lambda t: 1/((t+4)*sqrt((t+1)*(t+2)*(t+3)))
        >>> 1.5*quad(f, [0,inf])
        0.2398480997495677621758616696
        >>> elliprj(1,2+1j,3,4-2j)
        (0.2168889060146334987399519653 + 0.04081912627366673332369511511j)
        >>> f = lambda t: 1/((t+4-2j)*sqrt((t+1)*(t+2+1j)*(t+3)))
        >>> 1.5*quad(f, [0,inf])
        (0.2168889060146334987399519621 + 0.0408191262736667333236951143j)
...
def elliprd(ctx, x, y, z):
    ...
    **Examples**
        ...
        >>> mp.dps = 25; mp.pretty = True
        >>> elliprd(1,2,3)
        0.2904602810289906442326533888
        >>> elliprj(1,2,3,3)
        0.2904602810289906442326533888
    ...
    The so-called *second lemniscate constant*, a transcendental number::
        ...
        >>> elliprd(0,2,1)/3
        0.5990701173677961037199612466
        >>> extradps(25)(quad)(lambda t: t**2/sqrt(1-t**4), [0,1])
        0.5990701173677961037199612454
        >>> gamma('3/4')**2/sqrt(2*pi)
        0.5990701173677961037199612337
...
def elliprg(ctx, x, y, z):
    ...
    **Examples**
    ...
    Evaluation for real and complex arguments::
        ...
        3.141592653589793238462643402
        >>> elliprg(0,0.5,1)
        0.6753219405238377512600873677
        >>> chop(elliprg(1+j, 1-j, 2))
        1.172431327676416604532821569
...
def ellipf(ctx, phi, m):
    ...
    **Examples**
    ...
    Basic values and limits::
        ...
        1.226191170883517070813060957
        1.226191170883517070813060957
        >>> ellipf(pi/2, -0.5); ellipk(-0.5)
        1.415737208425956198892165985
        1.415737208425956198892165959
        >>> ellipf(pi/2+eps, 1); ellipf(-pi/2-eps, 1)
        inf
        inf
        >>> ellipf(1.5, 1)
        3.340677542798311003320812652

    Comparing with numerical integration::

        >>> z,m = 0.5, 1.25
        >>> ellipf(z,m)
        0.5287219202206327872978254939
    ...
    The arguments may be complex numbers::
        ...
        >>> ellipf(3j, 0.5)
        (0.0 + 1.713602407841590234804143255j)
        >>> ellipf(3+4j, 5-6j)
        (1.269131241950351323305740699 - 0.3561052815014558335412537747j)
        >>> z,m = 2+3j, 1.25
        >>> k = 1011
        >>> ellipf(z+pi*k,m); ellipf(z,m) + 2*k*ellipk(m)
        (4086.184383622179764082821439 - 3003.00353892374939654687064j)
        (4086.184383622179764082821386 - 3003.00353892374939654687064j)
    ...
    (see :func:`~mpmath.appellf1`)::
        ...
        >>> z,m = 0.5, 0.25
        >>> ellipf(z,m)
        0.5050887275786480788831082961
...
def ellipe(ctx, *args):
    ...
    **Examples for the complete integral**
    ...
    Basic values and limits::
        ...
        1.0
        >>> ellipe(-1)
        1.910098894513856008952381043
        >>> ellipe(2)
        (0.5990701173677961037199612466 + 0.5990701173677961037199612466j)
        >>> ellipe(inf)
        (0.0 + infj)
        >>> ellipe(-inf)
        inf
    ...
    Evaluation is supported for arbitrary complex `m`::
        ...
        >>> ellipe(0.5+0.25j)
        (1.36086868216312968271668675 - 0.1238733442561786843557315092j)
        >>> ellipe(3+4j)
        (1.49955352093334695433361223 - 1.577879007912758274533308938j)

    A definite integral::

        >>> quad(ellipe, [0,1])
        1.333333333333333333333333342
    ...
    **Examples for the incomplete integral**
    ...
    Basic values and limits::
        ...
        (2.0 + 3.0j)
        >>> ellipe(1,1); sin(1)
        0.8414709848078965066525023183
        0.8414709848078965066525023183
        >>> ellipe(pi/2, -0.5); ellipe(-0.5)
        1.751771275694817862026501842
        1.751771275694817862026501842
        >>> ellipe(pi/2, 1); ellipe(-pi/2, 1)
        1.0
        -1.0
        >>> ellipe(1.5, 1)
        0.9974949866040544309417233666

    Comparing with numerical integration::

        >>> z,m = 0.5, 1.25
        >>> ellipe(z,m)
        0.4740152182652628394264449192
    ...
    The arguments may be complex numbers::
        ...
        >>> ellipe(3j, 0.5)
        (0.0 + 7.551991234890371873502104778j)
        >>> ellipe(3+4j, 5-6j)
        (24.15299022574220502424466332 + 75.25036704803259974181559911j)
        >>> k = 35
        >>> z,m = 2+3j, 1.25
        >>> ellipe(z+pi*k,m); ellipe(z,m) + 2*k*ellipe(m)
        (48.30138799412005235090765763 + 17.47255216721987688224356857j)
        (48.30138799412005235090765681 + 17.47255216721987688224356775j)
    ...
    (see :func:`~mpmath.appellf1`)::
        ...
        >>> z,m = 0.5, 0.25
        >>> ellipe(z,m)
        0.4950017030164151928870375471
        >>> sin(z)*appellf1(0.5,0.5,-0.5,1.5,sin(z)**2,m*sin(z)**2)
        0.4950017030164151928870375536
...
def ellippi(ctx, *args):
    ...
    **Examples for the complete integral**
    ...
    Some basic values and limits::
        ...
        0.9555039270640439337379334377
        0.9555039270640439337379334377
        >>> ellippi(inf,2)
        0.0
        >>> ellippi(2,inf)
    ...
    Evaluation in terms of simpler functions::
        ...
        1.956616279119236207279727026
        1.956616279119236207279727
        >>> ellippi(3,0); pi/(2*sqrt(-2))
        (0.0 - 1.110720734539591561753970245j)
        (0.0 - 1.110720734539591561753970271j)
    ...
    **Examples for the incomplete integral**
    ...
    Basic values and limits::
        ...
        0.2513040086544925794134591284
        0.2513040086544925794134591284
        >>> ellippi(1,1,1); (log(sec(1)+tan(1))+sec(1)*tan(1))/2
        2.054332933256248668692451902
        2.05433293325624866869245185
        >>> ellippi(0.25, 53*pi/2, 0.75); 53*ellippi(0.25,0.75)
        135.2408687578908407550579568
        135.2408687578908407550579535
        >>> ellippi(0.5,pi/4,0.5); 2*ellipe(pi/4,0.5)-1/sqrt(3)
        0.9190227391656969903987269211
        0.9190227391656969903987269341

    Complex arguments are supported::

        >>> ellippi(0.5, 5+6j-2*pi, -7-8j)
        (-0.3612856620076747660410167109 + 0.5217735339984807829755815037j)

...
def differint(f, x, n=1, x0=0):
    ...
    **Examples**
    ...
    the following gives a half-derivative (order 0.5)::
        ...
        >>> mp.dps = 15
        >>> x = mpf(3); p = 2; n = 0.5
        >>> print differint(lambda t: t**p, x, n)
        7.81764019044672
        >>> print gamma(p+1)/gamma(p-n+1) * x**(p-n)
        7.81764019044672
    ...
    is set to `-\infty` to avoid nonzero endpoint terms.)::
        ...
        >>> print differint(lambda x: exp(pi*x), -1.5, 3)
        0.278538406900792
        >>> print exp(pi*-1.5) * pi**3
        0.278538406900792
        >>> print differint(lambda x: exp(pi*x), 3.5, -3, -inf)
        1922.50563031149
        >>> print exp(pi*3.5) / pi**3
        1922.50563031149
    ...
    Riemann-Liouville differintegral::
        ...
        >>> c = pi
        >>> n = 1+2*j
        >>> print differint(lambda x: exp(c*x), x, n)
        (-123295.005390743 + 140955.117867654j)
        >>> print x**(-n) * exp(c)**x * (x*c)**n * gammainc(-n, 0, x*c) / gamma(-n)
        (-123295.005390743 + 140955.117867654j)

...
def jtheta(ctx, n, z, q, derivative=0):
    ...
    if abs(q) > THETA_Q_LIM:
        raise ValueError('abs(q) > THETA_Q_LIM = %f' % THETA_Q_LIM)
...
def jsn(ctx, u, m):
    ...
    try:
        ctx.prec += extra
        q = ctx.calculate_nome(ctxsqrt(m))
        v3 = ctx.jtheta(3, zero, q)
        v2 = ctx.jtheta(2, zero, q)        # mathworld says v4
        arg1 = u / (v3*v3)
...
def jcn(ctx, u, m):
    ...
    try:
        ...
        v3 = ctx.jtheta(3, zero, q)
        v2 = ctx.jtheta(2, zero, q)
        v04 = ctx.jtheta(4, zero, q)
        arg1 = u / (v3*v3)
        v1 = jtheta(2, arg1, q)
        v4 = jtheta(4, arg1, q)
        cn = (v04/v2)*(v1/v4)

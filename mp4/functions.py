...
# XXX: context properties?
from rational import (mpq_1, mpq_0, mpq_1_2, mpq_3_2, mpq_1_4,
    mpq_1_16, mpq_3_16, mpq_5_2, mpq_3_4, mpq_7_4, mpq_5_4)
...
def hyper(ctx, a_s, b_s, z, **kwargs):
    ...
    coeffs = map(ctx.mpf_or_rational, a_s+b_s)
        ...
        while 1:
            ...
            for w_s, c_s, alpha_s, beta_s, a_s, b_s, z in terms:
                ...
                for w, c in zip(w_s, c_s):
                    v *= convert(w) ** c
                    #if ctx.prec > 300:
                    #if abs((convert(w) ** c) - complex(convert(w)) ** complex(convert(c))) > 0.001:
                    #    print "DUMB"
                    #    print w
                    #    print c
                    #    print convert(w) ** c, complex(convert(w)) ** complex(convert(c))
...
def hyp2f2(ctx,a1,a2,b1,b2,z,**kwargs):
    a1, a1type = ctx.mpf_or_rational(a1)
    a2, a2type = ctx.mpf_or_rational(a2)
    b1, b1type = ctx.mpf_or_rational(b1)
    b2, b2type = ctx.mpf_or_rational(b2)
    z = ctx.convert(z)
...
def hyp2f3(ctx,a1,a2,b1,b2,b3,z,**kwargs):
    ...
    b1, b1type = ctx.mpf_or_rational(b1)
    b2, b2type = ctx.mpf_or_rational(b2)
    b3, b3type = ctx.mpf_or_rational(b3)
    z = ctx.convert(z)
...
def hyperu(ctx, a,b,z):
    ...
    if not z:
        if ctx.re(b) <= 1:
            ...
            return ctx.inf + z
    bb = 1+a-b
    bb, bbtype = ctx.mpf_or_rational(bb)
    try:
        orig = ctx.prec
        try:
            ctx.prec += 10
...
def fresnels(ctx, z):
    ...
    return ctx.pi*z**3/6*ctx.hypsum(1,2,('Q','Q','Q'),\
        [mpq_3_4,mpq_3_2,mpq_7_4],-ctx.pi**2*z**4/16)
...
def fresnelc(ctx, z):
    ...
    return z*ctx.hypsum(1,2,('Q','Q','Q'),\
        [mpq_1_4,mpq_1_2,mpq_5_4],-ctx.pi**2*z**4/16)
